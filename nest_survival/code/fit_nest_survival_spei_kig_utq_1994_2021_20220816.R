...
run.spei <- function()
{
  ...
  # 3. site year init model
  S.syi = list(formula = ~ Site + Year + Init)
  
  # 4. site year nestage model
  S.syn = list(formula = ~ Site + Year + NestAge)
  
  # 5. site year init nestage model
  S.syin = list(formula = ~ Site + Year + Init + NestAge)
  
  # 6. site year init siteXnestage model
  S.syi.sXi = list(formula = ~ Site + Year + Init + Site*Init)
  
  # 7. site year nestage siteXnestage model
  S.syn.sXn = list(formula = ~ Site + Year + NestAge + Site*NestAge)
  
  # 8. site year init nestage siteXinit model
  S.syin.sXi = list(formula = ~ Site + Year + Init + NestAge + Site*Init)
  
  # 9. site year init nestage siteXnestage model
  S.syin.sXn = list(formula = ~ Site + Year + Init + NestAge + Site*NestAge)
  
  # 10. site year init nestage siteXinit siteXnestage model
  S.syin.sXi.sXn = list(formula = ~ Site + Year + Init + NestAge + Site*Init + Site*NestAge)
  
  # 11. siteXyear model
  S.sXy = list(formula = ~ -1 + Site:Year)
  
  # 12. siteXyear init model
  S.sXy.i = list(formula = ~ -1 + Site:Year + Init)
  
  # 13. siteXyear nestage model
  ...
  # 16. siteXyear site init nestage siteXinit model
  #S.sXy.sin.sXi = list(formula = ~ -1 + Site:Year + Site + Init + NestAge + Site:Init)
  
  # 17. siteXyear site init nestage siteXnestage model
  #S.sXy.sin.sXn = list(formula = ~ -1 + Site:Year + Site + Init + NestAge + Site:NestAge)
  
  # 18. siteXyear site init nestage siteXinit siteXnestage model
  S.sXy.sin.sXi.sXn = list(formula = ~ -1 + Site:Year + Site:Init + Site:NestAge)
...
spei.results$S.sy #2
spei.results$S.syi #3
spei.results$S.syn #4
spei.results$S.syin #5
spei.results$S.syi.sXi #6 
spei.results$S.syn.sXn #7
spei.results$S.syin.sXi #8
spei.results$S.syin.sXn #9
spei.results$S.syin.sXi.sXn #10
spei.results$S.sXy #11
spei.results$S.sXy.i #12
spei.results$S.sXy.n #13
spei.results$S.sXy.sXi #14
spei.results$S.sXy.sXn #15 this did not run
#spei.results$S.sXy.sin.sXi #16
#spei.results$S.sXy.sin.sXn #17
spei.results$S.sXy.sin.sXi.sXn
